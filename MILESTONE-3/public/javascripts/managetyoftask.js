    ...
    xhttp.onreadystatechange = function() {
        if (this.status == 200) {
            if (this.responseText != 'unsuccess') {
                ...
                taskText += '<option disabled selected value="0">CHOOSE TYPE OF TASK</option>';
                for (var index = 0; index < taskTitle.task.length; index++) {
                    taskText += '<option>' + taskTitle.task[index].title + '</span>';

                }
                document.getElementById('serviceQual').innerHTML = taskText;
            }
        }
...
function managetyoftask() {
    ...
    xhttp.open("POST", "/managetyoftask", true);
    xhttp.setRequestHeader("Content-type", "application/json");
    xhttp.send(JSON.stringify({
